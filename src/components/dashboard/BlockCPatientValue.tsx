...
export function BlockCPatientValue() {
    ...
    const isNew = (t: any) => {
      if (Array.isArray(t.procedures))
        return t.procedures.some(
          (p: any) => typeof p === 'string' && p.toLowerCase().includes('primeira consulta'),
        )
      if (typeof t.procedures === 'string')
        return t.procedures.toLowerCase().includes('primeira consulta')
      return false
    }
